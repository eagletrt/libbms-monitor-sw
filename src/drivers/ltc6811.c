    ...
    uint8_t * out)
{
    ...
    // Get command
    Ltc681xCommand cmd = WRCFGA;

    // Encode command with corresponding PEC
    _ltc681x_cmd_encode(cmd, false, 0U, out);
    size_t encoded = LTC681X_CMD_BYTE_COUNT + LTC681X_PEC_BYTE_COUNT;

    // Encode data with corresponsing PEC
    for (size_t i = 0; i < chain->count; ++i) {
        Ltc681xCfgr * cfg = &(config[chain->count - i - 1]);
        ...
        // Add PEC
        _ltc681x_pec_calc(out + encoded, LTC681X_REG_BYTE_COUNT);
        encoded += LTC681X_REG_BYTE_COUNT + LTC681X_PEC_BYTE_COUNT;
    }
    return encoded;
}
    ...
    uint8_t * out)
{
    ...
    // Get command
    Ltc681xCommand cmd = RDCFGA;

    // Encode command with corresponding PEC
    _ltc681x_cmd_encode(cmd, false, 0U, out);
    return LTC681X_CMD_BYTE_COUNT + LTC681X_PEC_BYTE_COUNT;
}
    ...
    Ltc681xCfgr * out)
{
    ...
    const size_t byte_count = LTC681X_REG_BYTE_COUNT + LTC681X_PEC_BYTE_COUNT;
    for (size_t i = 0; i < chain->count; ++i) {
        // Check PEC validity
        if (_ltc681x_pec_check(data + off, byte_count)) {
    ...
    uint16_t * out)
{
    ...
    for (size_t i = 0; i < chain->count; ++i) {
        ...
        if (_ltc681x_pec_check(data + off, byte_count)) {
            ...
            for (size_t j = 0; j < LTC681X_REG_CELL_COUNT; ++j) {
                size_t cell_off = off + j * sizeof(uint16_t);
                out[i * LTC681X_REG_CELL_COUNT + j] = data[cell_off] | ((uint16_t)data[cell_off + 1] << 8);
            }
            decoded += byte_count;
        }
        off += byte_count;
    ...
    uint16_t * out)
{
    ...
    for (size_t i = 0; i < chain->count; ++i) {
        ...
        if (_ltc681x_pec_check(data + off, byte_count)) {
            ...
            for (size_t j = 0; j < LTC681X_REG_AUX_COUNT; ++j) {
                size_t cell_off = off + j * sizeof(uint16_t);
                out[i * LTC681X_REG_AUX_COUNT + j] = data[cell_off] | ((uint16_t)data[cell_off + 1] << 8);
            }
            decoded += byte_count;
        }
        off += byte_count;
    ...
    uint8_t * out)
{
    ...
    // Encode data with corresponsing PEC
    for (size_t i = 0; i < chain->count; ++i) {
        for (size_t byte = 0; byte < LTC681X_REG_BYTE_COUNT; ++byte) {
            size_t index = i * LTC681X_SCTL_COUNT + byte * 2;
    ...
    uint8_t * out)
{
    ...
    for (size_t i = 0; i < chain->count; ++i) {
        ...
        if (_ltc681x_pec_check(data + off, byte_count)) {
            // For each data byte
            for (size_t byte = 0; byte < LTC681X_REG_BYTE_COUNT; ++byte) {
                size_t index = i * LTC681X_PWM_COUNT + byte * 2;
                out[index] = data[off + byte] & 0x0F;
                out[index + 1] = data[off + byte] >> 4;
            }
            decoded += byte_count;
...
{ 
    ...
    // Get command
    Ltc681xCommand cmd = ADCV;
    cmd = _ltc681x_cmd_set_md(cmd, mode);
    cmd = _ltc681x_cmd_set_dcp(cmd, dcp);
    cmd = _ltc681x_cmd_set_ch(cmd, cells);
...
{ 
    ...
    // Get command
    Ltc681xCommand cmd = ADOW;
    cmd = _ltc681x_cmd_set_md(cmd, mode);
    cmd = _ltc681x_cmd_set_pup(cmd, mode);
    cmd = _ltc681x_cmd_set_dcp(cmd, dcp);
    cmd = _ltc681x_cmd_set_ch(cmd, cells);
...
{ 
    ...
    // Get command
    Ltc681xCommand cmd = CVST;
    cmd = _ltc681x_cmd_set_md(cmd, mode);
    cmd = _ltc681x_cmd_set_st(cmd, test_mode);
    ...
    uint8_t * out)
{
    ...
    // Get command
    Ltc681xCommand cmd = ADAX;
    cmd = _ltc681x_cmd_set_md(cmd, mode);
    cmd = _ltc681x_cmd_set_chg(cmd, gpios);
    ...
    uint8_t * out)
{
    ...
    // Get command
    Ltc681xCommand cmd = ADSTAT;
    cmd = _ltc681x_cmd_set_md(cmd, mode);
    cmd = _ltc681x_cmd_set_chst(cmd, groups);
    ...
    uint8_t * out)
{
    ...
    // Encode data with corresponsing PEC
    for (size_t i = 0; i < chain->count; ++i) {
        Ltc681xComm * data = &(comm[chain->count - i - 1]);
    ...
    Ltc681xComm * out)
{
    ...
    for (size_t i = 0; i < chain->count; ++i) {
        ...
        if (_ltc681x_pec_check(data + off, byte_count)) {
            ...
            for (size_t j = 0; j < LTC681X_COMM_DATA_COUNT; ++j) 
    ...
    uint8_t * out)
{
    ...
    // Add dummy data
    memset(out + encoded, 0xFF, LTC681X_STCOMM_CYCLES);
    encoded += LTC681X_STCOMM_CYCLES;
    return encoded;
}
